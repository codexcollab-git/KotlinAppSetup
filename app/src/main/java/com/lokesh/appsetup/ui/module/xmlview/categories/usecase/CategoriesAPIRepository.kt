...
interface CategoriesAPIRepository {
    fun getCategories(): Flow<Result<ArrayList<String>?>>
...
class DefaultCategoriesRepo @Inject constructor(
    ...
    override fun getCategories(): Flow<Result<ArrayList<String>?>> {
        return flow {
            emit(Result.loading())
    ...
    override fun insertCategories(categories: List<String>?) {
        doInBackground {
